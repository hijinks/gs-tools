...
for q = 1:length(surfaces)
    ...
    distances_km_norm = distances_km./max(distances_km);
    % Fit model to data.
    [fitresult, gof] = fit(distances_km_norm, means', ft, opts );

    d = plot(distances_km_norm, means', ['-' styles{q}], 'Color', line_colours{q});
    hold on;
    
    sternberg = fitresult.a*exp(full_distances_norm.*-fitresult.b);
    h = plot(full_distances_norm, sternberg, ['--'], 'Color', line_colours{q}, 'LineWidth', 1);
...
backthrustq2c_distance_norm = backthrustq2c_distance./max(backthrustq2c_distance);
% Backthrst
back_means = plot(backthrustq2c_distance_norm, backthrustq2c_mean, ['-d'], 'Color', 'k');
hold on;
...
opts.StartPoint = [30 0.8033];
opts.Upper = [Inf 0.9];
[fitresult, gof] = fit(backthrustq2c_distance_norm, backthrustq2c_mean, ft, opts );

sternberg_mitch_backthrust = fitresult.a*exp(full_distances_norm.*-fitresult.b);
back_fit = plot(full_distances_norm, sternberg_mitch_backthrust, ['--'], 'Color', 'k');

backthrustlabel = 'Backthrust Canyon Q2c';
backthrust_eqn = [num2str(a) 'e^{-' num2str(b) 'x}'];

% MOONLIGHT
moonq2c_distance_norm = moonq2c_distance./max(moonq2c_distance);

moon_means = plot(moonq2c_distance_norm, moonq2c_mean, ['-^'], 'Color', [.5,.5,.5]);
hold on;
...
opts.StartPoint = [30 0.8033];
opts.Upper = [Inf 0.9];
[fitresult, gof] = fit(moonq2c_distance_norm, moonq2c_mean, ft, opts );

sternberg_mitch_moonlight = fitresult.a*exp(full_distances_norm.*-fitresult.b);
moon_fit = plot(full_distances_norm, sternberg_mitch_moonlight, ['--'], 'Color', [.5,.5,.5]);
...
print(f, '-dpdf', ['pdfs/sternberg_nor' '.pdf'])
