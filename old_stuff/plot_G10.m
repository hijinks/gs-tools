...
for pppp=1:length(plot_distances)
    ...
    y_data84 = plot_d84{pppp};
    y_data50 = plot_d50{pppp};
    y_datamean = plot_mean{pppp};
    [xsorted, I] = sort(x_data)
    ysortedmean = y_datamean(I)
    ysorted84 = y_data50(I)
    ysorted50 = y_data84(I)
    
    p = polyfit(xsorted,ysorted84,1)
    fit84 = polyval(p,xsorted);
    p = polyfit(xsorted,ysorted50,1)
    fit50 = polyval(p,xsorted);
    p = polyfit(xsorted,ysortedmean,1)
    fitmean = polyval(p,xsorted);
    title(surface_names{pppp});
    sname = surface_names(pppp);
    surface_data.(sname{1}) = struct('d84',ysorted84,...
        'd50',ysorted50,'mean',ysortedmean,'distance',xsorted,...
        'fit84',fit84,'fit50',fit50,'fitmean',fitmean);
end
