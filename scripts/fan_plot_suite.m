...
for fn=1:length(fannames)
    ...
    for sn=1:length(s_names)
        ...
        bigval(1:length(surface_wolman_all), 1) = cell2mat(surface(:,2));
        surface_wolmans = [surface_wolmans,bigval];
    ...
    boxplot(surface_wolmans, 'ExtremeMode', 'compress');
    hold on;
    ...
    print(surface_averages_figure, '-depsc', [output_path fannames{fn} '_surface_averages' '.eps'])
