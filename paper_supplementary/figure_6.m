...
for j=1:(length(dir_search)),
    ...
    if strcmp(ext,'.csv') > 0
        ...
        if isempty(fname) < 1
            ...
            formatSpec = '%f%f%f%f%f%f%f%f%f%q%q%q%q%q%q%q%q%q%q%q%q%q%q%[^\n\r]';
...
subplot(2,3,2);
gscatter(1:1:length(bg),bg,fan_categories', colors, symbols);
...
subplot(2,3,4);
gscatter(1:1:length(C1),C1,fan_categories', colors, symbols, '', 'off');
...
print(f, '-dpdf', ['pdfs/figure_6.pdf'])
